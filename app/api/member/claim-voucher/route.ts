...
// Tiers tukar poin → voucher
const TIERS = [
  { points: 20, discountAmount: 5000, label: "20 poin → diskon Rp5.000" },
  { points: 50, discountAmount: 20000, label: "50 poin → diskon Rp20.000" },
  { points: 100, discountAmount: 50000, label: "100 poin → diskon Rp50.000" },
  { points: 200, discountAmount: 120000, label: "200 poin → diskon Rp120.000" },
] as const;
...
export async function POST(request: NextRequest) {
  ...
  if (totalPoints < tier.points) {
    return NextResponse.json({ error: `Poin tidak cukup. Kamu punya ${totalPoints} poin.` }, { status: 400 });
  }

  // Buat kode unik
  const code = `POIN-${randomBytes(4).toString("hex").toUpperCase()}`;

  // Expire 30 hari dari sekarang
