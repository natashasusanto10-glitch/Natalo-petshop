...
export async function POST(request: Request) {
  ...
  if (!apiKey) {
    return NextResponse.json({
      rates: dummyRates(),
      message: "BITESHIP_API_KEY belum diisi — pakai data dummy.",
    });
  }
  if (!originPostal) {
    return NextResponse.json(
  ...
  if (!destinationPostal) {
    return NextResponse.json(
      { message: "Kode pos tujuan harus diisi." },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    const rates: RateOption[] = pricing.map((p) => {
      ...
      return {
        ...
        service_type: normalizeServiceType(p.service_type, p.duration),
        price,
...
function normalizeServiceType(raw?: string, duration?: string): string {
  const t = (raw ?? "").toLowerCase();
  if (["instant", "same_day", "next_day", "regular", "economy"].includes(t)) return t;
