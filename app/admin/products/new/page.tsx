...
export default async function AdminProductNewPage() {
  ...
  async function createProduct(formData: FormData) {
    ...
    const memberPrice = formData.get("memberPrice") ? parseInt(String(formData.get("memberPrice")), 10) : null;
    ...
    await prisma.product.create({
      data: { name, slug, description, price, memberPrice, stock, weightGram, imageUrl, categoryId },
    });
    ...
    <div className="mx-auto max-w-2xl px-4 py-10">
      ...
      <form action={createProduct} className="mt-8 space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Harga (Rp)" name="price" type="number" required placeholder="50000" />
          <Field label="Harga member (Rp)" name="memberPrice" type="number" placeholder="45000" />
        </div>
