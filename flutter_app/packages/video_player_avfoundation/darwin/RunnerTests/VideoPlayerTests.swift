...
@MainActor struct VideoPlayerTests {
  ...
  @Test func frameOutputCallbackCountsOnlyNewNonNullBuffers() throws {
    let mockVideoOutput = TestPixelBufferSource()
    let videoPlayerPlugin = try createInitializedPlugin(
      avFactory: StubFVPAVFactory(pixelBufferSource: mockVideoOutput))
    let identifiers = try videoPlayerPlugin.createTexturePlayer(
    ...
    let player = try #require(
      videoPlayerPlugin.playersByIdentifier[identifiers.playerId]
        as? FVPTextureBasedVideoPlayer)
    var observedCounts: [Int64] = []
    player.frameOutputCallback = { frameCount, _, _ in
      observedCounts.append(frameCount)
    }

    var firstBuffer: CVPixelBuffer?
    CVPixelBufferCreate(nil, 1, 1, kCVPixelFormatType_32BGRA, nil, &firstBuffer)
    mockVideoOutput.pixelBuffer = firstBuffer
    player.copyPixelBuffer()
    player.copyPixelBuffer()

    var secondBuffer: CVPixelBuffer?
    CVPixelBufferCreate(nil, 1, 1, kCVPixelFormatType_32BGRA, nil, &secondBuffer)
    mockVideoOutput.pixelBuffer = secondBuffer
    player.copyPixelBuffer()

    #expect(observedCounts == [1, 2])

    var error: FlutterError?
    player.disposeWithError(&error)
    #expect(error == nil)
    #expect(player.frameOutputCallback == nil)
  }

  #if os(iOS)
    @Test func frameOutputStreamRejectsLateCallbackAfterPlayerRemoval() async throws {
      let stream = FrameOutputEventStream()
      var frameCounts: [Int64] = []
      _ = stream.onListen(withArguments: nil) { event in
        if let map = event as? [String: Any], let count = map["frameCount"] as? Int64 {
          frameCounts.append(count)
        }
      }
      let staleGeneration = stream.registerPlayer(7)

      stream.recordOutput(
        playerId: 7, playerGeneration: staleGeneration, textureId: 70, frameCount: 1,
        mediaTimeUs: 100, monotonicTimeUs: 200)
      stream.removePlayer(7)
      stream.recordOutput(
        playerId: 7, playerGeneration: staleGeneration, textureId: 70, frameCount: 2,
        mediaTimeUs: 200, monotonicTimeUs: 300)

      let currentGeneration = stream.registerPlayer(7)
      stream.recordOutput(
        playerId: 7, playerGeneration: staleGeneration, textureId: 71, frameCount: 3,
        mediaTimeUs: 300, monotonicTimeUs: 400)
      stream.recordOutput(
        playerId: 7, playerGeneration: currentGeneration, textureId: 71, frameCount: 1,
        mediaTimeUs: 400, monotonicTimeUs: 500)

      #expect(frameCounts == [1, 1])
    }

    @Test func frameOutputStreamDropsThrottledOutputWithoutTrailingEmission() async throws {
      let stream = FrameOutputEventStream()
      var frameCounts: [Int64] = []
      _ = stream.onListen(withArguments: nil) { event in
        if let map = event as? [String: Any], let count = map["frameCount"] as? Int64 {
      ...
      let generation = stream.registerPlayer(8)

      for count in 1...3 {
        stream.recordOutput(
          playerId: 8, playerGeneration: generation, textureId: 80, frameCount: Int64(count),
          mediaTimeUs: Int64(count * 100), monotonicTimeUs: Int64(count * 200))
      }
      #expect(frameCounts == [1])

      try await Task.sleep(nanoseconds: 350_000_000)
      #expect(frameCounts == [1])

      stream.recordOutput(
        playerId: 8, playerGeneration: generation, textureId: 80, frameCount: 4,
        mediaTimeUs: 400, monotonicTimeUs: 800)
      #expect(frameCounts == [1, 4])
    }

    @Test func frameOutputStreamCancelAndInvalidateRejectPendingOutput() async throws {
      let stream = FrameOutputEventStream()
      var frameCounts: [Int64] = []
      _ = stream.onListen(withArguments: nil) { event in
        if let map = event as? [String: Any], let count = map["frameCount"] as? Int64 {
      ...
      let generation = stream.registerPlayer(9)
      stream.recordOutput(
        playerId: 9, playerGeneration: generation, textureId: 90, frameCount: 1,
        mediaTimeUs: 100, monotonicTimeUs: 200)
      stream.recordOutput(
        playerId: 9, playerGeneration: generation, textureId: 90, frameCount: 2,
        mediaTimeUs: 200, monotonicTimeUs: 300)
      ...
      stream.recordOutput(
        playerId: 9, playerGeneration: generation, textureId: 90, frameCount: 3,
        mediaTimeUs: 300, monotonicTimeUs: 400)
      stream.invalidate()
      stream.recordOutput(
        playerId: 9, playerGeneration: generation, textureId: 90, frameCount: 4,
        mediaTimeUs: 400, monotonicTimeUs: 500)

      #expect(frameCounts == [1, 3])
    }
  #endif
