...
public final class VideoPlayerPlugin: NSObject, FlutterPlugin, AVFoundationVideoPlayerApi {
  ...
  #if os(iOS)
    private let frameOutputEventStream: FrameOutputEventStream
    private let frameOutputEventChannel: FlutterEventChannel
  #endif
  ...
  init(
    ...
    #if os(iOS)
      let frameStream = FrameOutputEventStream()
      self.frameOutputEventStream = frameStream
      self.frameOutputEventChannel = FlutterEventChannel(
        name: "com.natalopetshop/video_rendered_frames", binaryMessenger: binaryMessenger)
    #endif
    super.init()
    #if os(iOS)
      frameOutputEventChannel.setStreamHandler(frameOutputEventStream)
    #endif
  }

  public func detachFromEngine(for registrar: FlutterPluginRegistrar) {
    #if os(iOS)
      frameOutputEventChannel.setStreamHandler(nil)
      frameOutputEventStream.invalidate()
    #endif
  ...
  func createTexturePlayer(options creationOptions: CreationOptions) throws -> TexturePlayerIds {
    ...
    #if os(iOS)
      let playerGeneration = frameOutputEventStream.registerPlayer(playerId)
      player.frameOutputCallback = { [weak self] frameCount, mediaTimeUs, monotonicTimeUs in
        self?.frameOutputEventStream.recordOutput(
          playerId: playerId, playerGeneration: playerGeneration, textureId: textureId,
          frameCount: frameCount,
          mediaTimeUs: mediaTimeUs, monotonicTimeUs: monotonicTimeUs)
      }
    #endif
  ...
  private func configurePlayer(
    ...
    player.onDisposed = { [weak self] in
      ...
      extraDisposeHandler?()
      #if os(iOS)
        strongSelf.frameOutputEventStream.removePlayer(playerId)
      #endif
