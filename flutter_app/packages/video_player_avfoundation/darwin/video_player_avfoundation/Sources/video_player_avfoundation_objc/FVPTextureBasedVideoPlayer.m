...
// Cumulative number of new pixel buffers copied for this player.
@property(nonatomic) int64_t outputBufferCount;
...
{
  FVPFrameOutputCallback _frameOutputCallback;
}

- (FVPFrameOutputCallback)frameOutputCallback {
  @synchronized(self) {
    return [_frameOutputCallback copy];
  }
}

- (void)setFrameOutputCallback:(FVPFrameOutputCallback)callback {
  @synchronized(self) {
    _frameOutputCallback = [callback copy];
  }
}
  ...
  if ([self.pixelBufferSource hasNewPixelBufferForItemTime:outputItemTime]) {
    ...
    if (buffer) {
      ...
      @synchronized(self) {
        ...
        if (_frameOutputCallback) {
          CMTime mediaTime =
              CMTIME_IS_VALID(itemTimeForDisplay) ? itemTimeForDisplay : outputItemTime;
          int64_t mediaTimeUs =
              CMTimeConvertScale(mediaTime, 1000000, kCMTimeRoundingMethod_Default).value;
          int64_t monotonicTimeUs = (int64_t)(currentTime * 1000000.0);
          _frameOutputCallback(self.outputBufferCount, mediaTimeUs, monotonicTimeUs);
        }
      }
    }
  }
