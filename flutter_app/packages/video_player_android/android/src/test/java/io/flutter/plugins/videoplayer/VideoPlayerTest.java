...
public final class VideoPlayerTest {
  ...
  @Test
  public void clearsFrameOutputListenerBeforeRelease() {
    VideoPlayer videoPlayer = createVideoPlayer();
    FrameOutputEventChannel.PlayerListener listener =
        mock(FrameOutputEventChannel.PlayerListener.class);

    videoPlayer.setFrameOutputListener(listener);
    videoPlayer.dispose();

    InOrder inOrder = inOrder(listener, mockExoPlayer);
    inOrder.verify(listener).dispose();
    inOrder.verify(mockExoPlayer).setVideoFrameMetadataListener(null);
  ...
  public void playsAndPausesProvidedMedia() {
    ...
    videoPlayer.play();
    verify(listener).setActive(true);
    verify(mockExoPlayer).play();

    videoPlayer.pause();
    verify(listener).setActive(false);
