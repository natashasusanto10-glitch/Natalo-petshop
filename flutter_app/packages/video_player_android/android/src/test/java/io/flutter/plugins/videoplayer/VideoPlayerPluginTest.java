...
public class VideoPlayerPluginTest {
  ...
  private FrameOutputEventChannel getFrameOutputEventChannel() throws Exception {
    final Field field = VideoPlayerPlugin.class.getDeclaredField("frameOutputEventChannel");
    field.setAccessible(true);
    return (FrameOutputEventChannel) field.get(plugin);
  }

  private EventChannel.EventSink getSink(FrameOutputEventChannel channel) throws Exception {
    final Field field = FrameOutputEventChannel.class.getDeclaredField("sink");
    field.setAccessible(true);
    return (EventChannel.EventSink) field.get(channel);
  }
  ...
  @Test
  public void initializePreservesGlobalFrameOutputSubscriber() throws Exception {
    FrameOutputEventChannel channel = getFrameOutputEventChannel();
    EventChannel.EventSink sink = mock(EventChannel.EventSink.class);
    channel.onListen(null, sink);

    plugin.initialize();

    assertSame(sink, getSink(channel));
  }
