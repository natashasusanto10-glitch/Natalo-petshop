...
class _MemberPostsScreenState extends State<MemberPostsScreen> {
  ...
  Future<void> _loadPosts() async {
    ...
    final result = await feedService.fetchMyPosts(filter: _tabs[_tabIndex].filter);
    if (!mounted) return;
    setState(() {
      _posts = result;
      _loading = false;
  ...
  void _openUpload() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Upload video tersedia di web. Buka natalopetshop.com untuk lanjut upload.'),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    if (post.status == MyFeedPostStatus.active) ...[
                      const SizedBox(height: 4),
                      Text(
