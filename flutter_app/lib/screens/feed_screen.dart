...
class _FeedScreenState extends State<FeedScreen>
  ...
  // pauseAll/resumeAll) dan dipakai widget item (attach/setActive/detach).
  ...
  void didChangeDependencies() {
    ...
    // opaque menutup Feed, resume saat kembali.
  ...
  Future<void> _bootstrapFromCache() async {
    try {
      ...
      if (cached.isNotEmpty && _posts.isEmpty) {
        ...
        // Mode managed TIDAK punya jalur self-init widget
        // (ownsController:false — widget menunggu coordinator attach). Tanpa
        // ini, render offline-cache-only (network gagal/lambat) tak pernah
        // men-attach item aktif ke coordinator → layar kosong/beku.
  ...
  /// window video-only via `coordinator.setPreloadWindow` memakai `offsets`
  /// dari `adaptiveVideoPreloadPolicy`.
  ...
  /// Alias — internal callers (_loadInitial, _onPageChanged) tetap pakai
  /// nama _preloadNext.
    ...
    return VisibilityDetector(
      ...
      child: FeedReelsCommentSurface(
        ...
        child: ColoredBox(
          ...
          //
          // Tag orang TIDAK lagi ditampilkan di Feed (badge + pill dipindah
          // ke halaman Postingan detail sesuai keputusan produk) — Feed
          // fokus ke media + social proof tanpa ikon tag yang menimpa baris
          // "Disukai oleh".
          child: GestureDetector(
            behavior: HitTestBehavior.opaque,
