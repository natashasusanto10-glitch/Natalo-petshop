...
**Files:**
- Modify: `flutter_app/lib/features/feed/transition/post_page_zoom_back_gesture.dart` (add a hero-based preview resolver/surface alongside the existing card one, or retarget it)
...
- `debugCurrentBackFrame`/continuity: replace the `PostPageBackFrame`(rect+radius) interpolation with `PostPageHeroFrame` interpolation (lerp scale/offset/clip); keep "continues from exact current transform" (no restart) as the asserted guarantee.
