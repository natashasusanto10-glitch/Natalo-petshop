...
3. **Avatar aktor** — notif sosial komentar/mention/like/share TIDAK mengirim foto aktor (slot `thumbnailUrl` diisi foto POST via `createFeedNotification` `lib/feed/notification-center.ts:143`). Follow (`sendFollowNotification` `lib/social/notifications.ts:25-118`) mengisi `thumbnailUrl` = foto follower (brand-safe via `lib/social/brand-user.ts`), sehingga di redesign foto follower muncul di KANAN (slot thumbnail) — sisi yang salah untuk avatar.
...
- `sendFeedPublishPush` (`lib/feed/publish-push.ts:127-138`): saat membuat baris `Announcement`, set `feedPostId: post.id` dan `thumbnailUrl: post.thumbnailUrl` (sumber post yang sama dipakai membangun push). Selaras dengan notif aktivitas feed lain.
- Pertimbangkan set `type`/`eventType` konsisten dgn feed (saat ini `type:"announcement"`). Minimal: pastikan `feedPostId` terisi; `type` boleh tetap agar tak mengubah pengelompokan tab (client tab "Aktivitas" match via keyword feed juga). Verifikasi tidak memecah `mapAnnouncement`'s `isFeed` logic (`route.ts:61`).

**Client:** tak ada. `_navigateForNotification` sudah mengutamakan `feedPostId` → `_openFeedPostInApp` (`notifications_screen.dart`).

**Test:** unit builder — baris hasil `sendFeedPublishPush` mengandung `feedPostId` + `thumbnailUrl` non-null untuk foto/video. Regресi: notif publish existing tetap terkirim.
...
- Di `_navigateForNotification` (`flutter_app/lib/screens/notifications_screen.dart`), sebelum cabang `/member/orders` list: kalau `url` mengandung `orderNumber` (query `orderNumber=` atau segমen path `buildOrderDetailPath`), ekstrak `orderNumber`, tampilkan spinner, `await orderService.fetchOrderDetail(orderNumber)`, lalu `Navigator.pushNamed('/member/order-detail', arguments: order)`. Gagal fetch → fallback `/member/orders` (persis pola `_openOrderByNumber` di `deep_link_service.dart:281-289` dan spinner pola `_openFeedPostInApp`).
- Ekstraksi `orderNumber`: reuse/among-parse dari `url`; tulis helper murni `String? extractOrderNumber(String url)` yang bisa diuji.
...
### P2a — Backend
- **Migration Prisma**: tambah kolom `Announcement.actorAvatarUrl String?`, `actorName String?`, `actorUsername String?` (`prisma/schema.prisma` model `Announcement`). Buat migration SQL (bukan sekadar `db push`) — lihat gotcha product-video schema drift.
- **`createFeedNotification`** (`lib/feed/notification-center.ts:87-181`): tambah param opsional `actor?: { avatarUrl, name, username }`; tulis ke tiga kolom baru. `thumbnailUrl` TETAP foto post (kanan). Jadi baris bisa punya keduanya: avatar aktor (kiri) + thumbnail post (kanan).
- **Builder sosial** (`lib/feed/activity-notifications.ts` komentar :69-83, reply :121-136, mention :195-217, like :309/:339, share :451): select `profilePhotoUrl,name,username,role` aktor; jalankan lewat `brandPhotoUrl(role, photo)` + `brandDisplayName(role, name)` (`lib/social/brand-user.ts`); teruskan ke `createFeedNotification.actor`. Like/share yang saat ini generik ("seseorang") → sertakan aktor (bila single-actor) atau tetap agregat (bila milestone).
- **Follow** (`lib/social/notifications.ts`): set `actorAvatarUrl` = foto follower (+ `actorName/username`). **Transisi aman:** JANGAN kosongkan `thumbnailUrl` follow sampai versi app minimum sudah membaca `actorAvatarUrl` — kalau backend P2 deploy saat app masih di P1b (yang baca avatar follow dari `thumbnailUrl`), follow kehilangan avatar. Aturan: isi `actorAvatarUrl` DAN pertahankan `thumbnailUrl`=foto follower untuk follow selama masa transisi; hapus `thumbnailUrl` follow di rilis backend berikutnya setelah adopsi app P2 memadai (atau biarkan — client P2 mengutamakan `actorAvatarUrl` dan follow tak punya konten kanan, jadi thumbnailNya bisa diabaikan client saat `eventType=='user_followed'`). Brand-guard sudah ada.
- **API** `mapAnnouncement` (`app/api/notifications/me/route.ts:38-73`): expose `actorAvatarUrl,actorName,actorUsername`.

### P2b — Client
- `AppNotification`: tambah `actorAvatarUrl,actorName,actorUsername` (parse + copyWith).
...
4. P2: notif komentar/mention/like/share/follow menampilkan foto aktor (brand-safe) di kiri; admin → identitas Natalo Petshop, bukan pemilik asli.
