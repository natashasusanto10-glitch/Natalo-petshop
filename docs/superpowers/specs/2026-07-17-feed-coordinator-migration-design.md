...
**Tanggal:** 2026-07-17
**Status:** DESAIN v4 — arsitektur matang (v2 menutup 6 gap dari review kedua; v3 menutup 3 gap dari review ketiga — race `clearActive()`, lease transient-gesture, 4 sekuens attach/detach/dispose; **v4 menutup 2 gap dari review keempat**: **detach video→foto WAJIB tak bersyarat** — bukan "kalau keluar window", karena attach=view-aktif dan pin=urusan terpisah; **barrier teardown asinkron KONKRET** — `detach()`/`_evict()` sinkron murni (fire-and-forget dispose) tak bisa "menunggu" `lease.end()` asinkron begitu saja, perlu field `cleanupInFlight` + guard eviction tambahan + re-evict setelah cleanup selesai — plus 1 kontradiksi kecil diperbaiki, klaim "widget tak tersentuh"). Belum ada kode ditulis, menunggu greenlight terpisah untuk eksekusi.
...
**Mekanisme yang benar (menambah satu field + satu guard, minim ripple):**
1. Tambah `bool cleanupInFlight = false` di `_SessionEntry`.
...
3. `detach(viewId, postId)`: `attachedViewIds.remove(viewId)` tetap **sinkron seketika** (identitas "siapa lagi merender" harus akurat langsung — TAK ditunda). TAPI kalau entry itu punya `TransientGestureLease` aktif (`isCurrent==true`): set `entry.cleanupInFlight = true` SEBELUM memanggil `_evict()` pass pertama (supaya entry lolos dari pembuangan pass ini), lalu `_enqueuePlayback(() async { await lease.end(); entry.cleanupInFlight = false; })` — begitu operasi asinkron ini selesai lewat antrean serial yang sama (`_playbackTail`), panggil `_evict()` SEKALI LAGI (pass kedua) supaya entry yang sekarang benar-benar tak lagi cleanup-in-flight bisa dibuang kalau memang sudah waktunya.
4. Kalau entry TIDAK punya lease aktif (kasus normal, mayoritas), `detach()` berjalan persis seperti sekarang — nol overhead tambahan untuk jalur yang tak melibatkan gesture.

**Test wajib (lease, diperluas):**
1. A di-`beginTransientGesture(doubleSpeed)` → sebelum `end()`, sesi A di-`detach()` (simulasikan swipe cepat) → assert **A TIDAK langsung ter-dispose** (`disposeCallCount==0` segera setelah `detach()` return, membuktikan barrier bekerja) → assert speed A ter-restore ke 1x DAN `lease.isCurrent == false` **setelah antrean serial di-flush** → assert A **sekarang** ter-dispose (`disposeCallCount==1`) barulah setelah cleanup selesai, bukan sebelumnya.
2. Panggil `lease.end()` (terlambat, sesudah kasus di atas) → assert no-op, tak ada efek pada sesi B yang sekarang aktif.

**File terdampak untuk kontrak ini:** `post_video_coordinator.dart` (interface + kelas lease + field `cleanupInFlight` di `_SessionEntry` + guard eviction + `detach()` bercabang), `video_player_session.dart` (implementasi), **3 file test fake** (harus implement method baru — kesempatan sekaligus menambah test kasus di atas).
...
2. **[PRASYARAT]** Perluas `PlaybackSession` (interface + `VideoPlayerSession` + 3 fake test) dengan `beginTransientGesture(kind) → TransientGestureLease` (`lease.end()`, bukan method terpisah di session), penuhi 4 jaminan di atas TERMASUK mekanisme barrier `cleanupInFlight` (§C) di `detach()`/`_evict()`. Ganti `_onLongPressStart`/`_onLongPressEnd` di `feed_video_post_view.dart` dari `ctrl.pause()/setPlaybackSpeed()` langsung ke intent lewat coordinator (simpan lease saat begin, panggil `lease.end()` saat end) saat `_managed` (bukan lagi early-return kosong).
...
- **Kematangan arsitektur:** v4 (dokumen ini) menutup 11 gap total dari empat putaran review — v2: urutan rollback, gerbang collision granular, kontrak `clearActive()`, kontrak transient-gesture (keputusan Opsi A), 4 kontradiksi internal, kontrak session-factory Feed. v3: race sinkron/async `clearActive()`, lease beridentitas (analog `VideoAudioClaim`), 4 sekuens attach/detach/dispose. v4: detach video→foto dibuat tak bersyarat (konsisten pola video→video), mekanisme barrier `cleanupInFlight` konkret untuk teardown asinkron (field baru di `_SessionEntry` + guard eviction + re-evict pass kedua) — menutup gap "requirement tanpa mekanisme" yang v3 nyatakan tapi belum bisa dieksekusi dengan `detach()`/`_evict()` yang sinkron. Pola berulang di keempat putaran: setiap review menemukan detail implementasi yang terlewat, bukan kesalahan arah — sinyal desainnya konvergen, bukan berputar.
- **Ukuran kerja:** sedang-besar — Langkah 1-2 (kontrak API `clearActive`+lease+barrier) makin jadi fondasi paling berisiko (3 putaran review berturut-turut menemukan race/gap halus di situ), **WAJIB review adversarial berbasis mutasi** sebelum lanjut ke langkah 3+, bukan sekadar review teks. Tetap bukan "quick patch"; 8 langkah bertahap, masing-masing PR terpisah dengan gerbang sendiri.
