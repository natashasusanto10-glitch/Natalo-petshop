...
**Tanggal:** 2026-07-17
**Status:** DESAIN v11 — arsitektur matang (v2: 6; v3: 3; v4: 2; v5: 3; v6: 2; v7: 1 fundamental; v8: 4/3 P1; v9: 6/4 P1; v10: 4/2 P1 — veto jadi lease-scoped, tapi hanya untuk jalur VETO. **v11 menutup 1 gap P1 tersisa dari review kesebelas**: jalur RESUME natural `end(allowResume:true)` A MASIH bump `_playbackGeneration` global tanpa gate. Kalau A tersusul `setActive(B)` selagi `end(true)` A masih di antrean, bump A yang telat membasikan generation B → B gagal autoplay (walau `_claimAndPlay(A)` sendiri sudah ditolak). v10 menutup sisi veto tapi lupa sisi resume — bump generation resume masih global. Fix: GATE SINKRON LENGKAP (`_resumeVetoed || !isCurrent || postId!=_activePostId || _suspended || _userPausedActive`) SEBELUM `++_playbackGeneration`; kalau A tak lagi eligible, return TANPA menyentuh generation global sama sekali → state B utuh. **Pola tiga putaran (v8→v9→v10→v11)**: setiap fix konkurensi di `_performEnd` menyisakan satu jalur global-state yang belum di-lease-scope. v11 diyakini menutup jalur terakhir. Belum ada kode ditulis, menunggu greenlight terpisah untuk eksekusi.
  ...
  Future<void> _performEnd() {
    ...
    return coordinator._enqueuePlayback(() async {
      try {
        ...
        // Poin 1 fix (review kesebelas): GATE SINKRON LENGKAP SEBELUM bump
        // generation. Bug v10: bump `_playbackGeneration` global tetap terjadi
        // walau A sudah tak aktif (tersusul `setActive(B)` selagi `end(true)` A
        // masih di antrean). Meski `_claimAndPlay(A)` lalu ditolak, bump-nya
        // TERLANJUR membasikan generation B yang baru dibuat `setActive(B)` →
        // B gagal autoplay. Veto sudah lease-scoped, TAPI bump RESUME masih
        // global. Solusi: JANGAN bump sama sekali kalau A tak lagi eligible —
        // cek SEMUA kondisi (veto, current, aktif, suspended, user-pause)
        // SINKRON, baru bump + resume. Kalau salah satu gagal, return tanpa
        // menyentuh `_playbackGeneration` → state global B utuh.
        if (_resumeVetoed ||
            !isCurrent ||
        ...
        // Sekarang aman: A terbukti masih post aktif & eligible SECARA SINKRON,
        // jadi bump generation ini murni intent resume A sendiri, tak men-cancel
        // video lain. `extraCanPlay` LEASE-SCOPED tetap dipasang sebagai
        // pertahanan pasca-await (veto yang datang SELAGI `_claimAndPlay` await).
...
16. **[P1 review kesebelas] Natural `end(true)` A TAK boleh membatalkan autoplay B**: A aktif → begin gesture A → panggil `lease_A.end(allowResume:true)` TAPI JANGAN flush dulu (masih di antrean, belum dapat giliran) → panggil `setActive(B)` (B jadi aktif, dapat generation baru) → BARU flush antrean penuh → assert **B autoplay (`play()`)**. Test ini gagal kalau `_performEnd` A masih bump `_playbackGeneration` global tanpa gate sinkron eligibility — bump A yang telat akan membasikan generation B. Bedakan dari #14 (yang menguji jalur `end(false)`/detach); #16 khusus jalur RESUME natural `end(true)` yang tetap bump generation di v10.
