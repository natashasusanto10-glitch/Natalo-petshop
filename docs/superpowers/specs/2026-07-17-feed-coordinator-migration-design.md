...
**Tanggal:** 2026-07-17
**Status:** DESAIN v5 — arsitektur matang (v2: 6 gap; v3: 3 gap — race `clearActive()`, lease transient-gesture, 4 sekuens attach/detach/dispose; v4: 2 gap — detach video→foto tak bersyarat, barrier teardown asinkron konkret; **v5 menutup 3 gap dari review kelima, dua di antaranya bug nyata**: **`_evict()` sendirian bisa membuat entry yatim permanen** — WAJIB dipasangkan dengan `_disposeStaleUnprotectedEntries()` (pola sudah ada di `setPreloadWindow:276-277`, `_evict()` punya early-return `<=maxSessions` yang membuatnya no-op untuk skenario Feed tipikal; **`cleanupInFlight` butuh generation counter**, bukan bool polos — `_SessionEntry` dipakai ulang per postId, siklus cleanup lama (swipe A→B→A→B) bisa menimpa status siklus baru; **capture `oldEntry` eksplisit sebelum `_activePostId=null`** dijadikan langkah bernomor tersendiri, bukan tersirat). Belum ada kode ditulis, menunggu greenlight terpisah untuk eksekusi.
...
- **Kematangan arsitektur:** v5 (dokumen ini) menutup 14 gap total dari lima putaran review. v2: urutan rollback, gerbang collision granular, kontrak `clearActive()`, kontrak transient-gesture (keputusan Opsi A), 4 kontradiksi internal, kontrak session-factory Feed. v3: race sinkron/async `clearActive()`, lease beridentitas, 4 sekuens attach/detach/dispose. v4: detach video→foto tak bersyarat, barrier teardown asinkron konkret. v5: pasangan `_disposeStaleUnprotectedEntries()`+`_evict()` (bug entry-yatim-permanen dari `_evict()` sendirian), `cleanupGeneration` (bukan bool, cegah race A→B→A→B), capture `oldEntry` eksplisit. **Pola lima putaran ini sendiri adalah sinyal penting**: setiap putaran menemukan detail implementasi konkurensi yang terlewat (bukan kesalahan arah arsitektur) — bidang yang paling banyak dikoreksi berulang-ulang adalah persis `§Kontrak API` (clearActive + lease + barrier), yang berarti area itu punya kompleksitas konkurensi tertinggi di seluruh migrasi dan LAYAK diberi waktu/perhatian ekstra saat implementasi nanti, bukan ditulis cepat lalu diperbaiki lewat hotfix.
- **Ukuran kerja:** sedang-besar — Langkah 1-2 (kontrak API `clearActive`+lease+barrier) adalah fondasi paling berisiko (4 putaran review berturut-turut menemukan race/gap halus di situ), **WAJIB review adversarial berbasis mutasi** sebelum lanjut ke langkah 3+, bukan sekadar review teks. Pertimbangkan bahkan menulis test-nya lebih dulu (TDD) mengingat kompleksitas konkurensinya. Tetap bukan "quick patch"; 8 langkah bertahap, masing-masing PR terpisah dengan gerbang sendiri.
