...
- **Webhook CF→Next.js (push balik)** `POST /api/chat/webhook`: **shared-secret/HMAC** header `x-chat-signature` (env, constant-time), body `{chatId, customerUserId, messageId, preview, senderName}` (**kontrak final Plan 2 Task 8 + Plan 3**; menggantikan draf awal `{customerId,lastMessageText,lastMessageType}`); **dedupe by `messageId`** — store: doc Firestore `customerChats/{chatId}/webhookSeen/{messageId}` (tanpa migrasi Prisma). Resolve `customerUserId → PushSubscription.where({userId:customerUserId, endpoint startsWith 'fcm:'})` → kirim via `lib/fcm.ts`.
...
- **Store**: doc Firestore `app_settings/chatConfig` `{chatEnabled:bool}` di tokochat — **sumber tunggal** (keputusan final 2026-07-07; menggantikan rencana awal tabel Prisma `AppConfig`, dibatalkan agar toggle instan tanpa migrasi/redeploy). **Owner menulisnya langsung dari NLCATTER** (layar Pengaturan Chat, Plan 6; rule owner-write di Plan 1 Task 7).
- **Customer app**: `GET /api/chat/config` → `{chatEnabled:bool, online:bool, hours}`, di-fetch saat app start + di-cache. `false` → sembunyikan `AppChatButton` & tombol chat detail produk; room existing bisa dibaca tapi composer nonaktif ("Chat sedang dalam pemeliharaan").
- **Proxy**: semua handler `api/chat/*` (customer & staff) BACA `app_settings/chatConfig.chatEnabled` di awal; mati → `503` generik.
- **NLCATTER**: owner toggle di Pengaturan Chat (Plan 6) menulis `app_settings/chatConfig`; staff lain BACA doc yang sama (rule staff-read) → banner "Fitur chat customer nonaktif sementara". **Tanpa mirror** — sumber = doc Firestore itu sendiri.
...
2. **Proxy Next.js** — `api/chat/*` (room transaksional, send text, send-image→UploadThing, fetch allowlist + paginasi, room-summary+unread, snapshot, auto-greeting/away + **auto-reopen**, resolve jam WIB), `api/chat/config` (kill-switch BACA `app_settings/chatConfig` + status online dari `chatHours`), `api/catalog/search` + `api/chat/staff-send-image` (auth §5.4 — **Plan 2.5**), `api/chat/webhook` (shared-secret + dedupe messageId), rate-limit store.
...
5. **NLCATTER customer inbox** — filter Customer, mode `isCustomer` (strip info + banner "sedang dibalas", attach tiles, ⋮, badge per-uid, tulis `typingStaff`), catatan internal, product picker + aturan stok + empty/error state, sheet detail, status +resolved/reopen copy (reopen otomatis = proxy), penolakan akses UI, **toggle `canHandleCustomer`** di `manage_users_screen.dart` (modify), banner kill-switch (baca `app_settings/chatConfig`). **Sakelar kill-switch + editor jam operasional owner → Plan 6** (layar Pengaturan Chat).
...
**`natalopetshopflutter/app/` (Next.js):** `api/chat/*` (proxy: room transaksional, send text/`send-image`→UploadThing, fetch allowlist + paginasi, room-summary+`unread`, snapshot, jam WIB, auto-greeting/away + auto-reopen), `api/chat/staff-send-image` + `api/catalog/search` (auth §5.4 — **Plan 2.5**), `api/chat/webhook` (shared-secret + dedupe di `webhookSeen`), `api/chat/config` (BACA `app_settings/chatConfig` kill-switch + status online dari `chatHours`), Admin SDK tokochat (`getTokochatFirestore`/`getTokochatAuth` + `verifyIdToken`), `lib/fcm.ts` (resolve `PushSubscription`), store rate-limit/idempotency, reuse `uploadToUT`/magic-byte. Kill-switch = doc Firestore (bukan Prisma `AppConfig`); `ChatThread`/`ChatMessage` lama sudah dihapus.

**`NLCHAT/` (NLCATTER Flutter):** mode `isCustomer` di `screens/chat_screen.dart` (attach mode customer → `api/chat/staff-send-image`; tulis `typingStaff`; banner "sedang dibalas") + filter di `chat_list_screen.dart`, `screens/customer_product_picker.dart` (baru, +empty/error state), sheet detail customer, render catatan internal + auto-message + resolved/reopen copy, kontrol status, penolakan akses UI, **`screens/manage_users_screen.dart` (modify — toggle `canHandleCustomer`)**, `screens/chat_settings_screen.dart` (baru — sakelar kill-switch + editor jam operasional, **Plan 6**, tulis `app_settings/chatConfig`+`chatHours`), banner kill-switch (baca `app_settings/chatConfig`), `models/customer_chat*.dart` (baru), `functions/index.js` (`notifyNewCustomerMessage` recipients owner+flag + webhook push-balik + log §11c), `firestore.rules` (+`customerChats` + **guard `canHandleCustomer` immutable CREATE&UPDATE `users`**, §5.3), `firestore.indexes.json` (+index). (`storage.rules` tak berubah.)
