...
async function callFonnte(phone, message) {
  ...
  // Log Fonnte success response — kalau OTP "tidak masuk" walau API success,
  // detail ini bantu trace: status "message in queue" vs delivered, ID pesan,
  // process timing, target normalized, dll. Tail terminal `next dev`.
  console.log("[whatsapp] sent ok", {
    target: phone,
    httpStatus: response.status,
    fonnte: typeof payload === "object" ? payload : { raw: payload },
  });
